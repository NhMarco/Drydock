...
const GITHUB: Deadline = { label: "GitHub", error: (status, message) => new GitHubError(status, message) };
...
export class GitHubClient {
  ...
  private send(url: string, accept: string, streamed = false): Promise<Response> {
    return fetchWithDeadline(url, this.headers(accept), this.config.upstreamTimeoutMs, GITHUB, streamed);
  }
  ...
  async listDirectory(path: string): Promise<ContentEntry[]> {
    ...
    const data = (await readBody(response.json(), GITHUB)) as unknown;
  ...
  async fetchRawText(path: string): Promise<string> {
    ...
    return await readBody(response.text(), GITHUB);
  }
